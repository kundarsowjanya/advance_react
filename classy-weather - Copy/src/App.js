...
class Input extends React.Component{
  render(){
    return(
      <div>
      <input type='text' placeholder='Search from location...' value={this.props.location} onChange={this.props.setLocation}/>
    </div>
    )
  }
}
    ...
    return(
     <div>
      ...
      <ul className='weather'>
        {
          dates?.map((date,index)=><Day date={date} max={max.at(index)} min={min.at(index)} code={codes.at(index)} key={date} isToday={index===0}/>)
        }
      </ul>
     </div>
    )
